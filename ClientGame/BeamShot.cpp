...
BeamShot::BeamShot(float X, float Y){
	//initialize beamShot position and speed
	position.X = X;
	position.Y = Y;
	//beamShotVel += shooterVel;
	//setup beamShot
	this->SetPosition(position);
	this->SetSize(1.0f);
	this->SetColor(1, 1, 1, 1); //(white and opaque so the texture comes through fully)
	this->ClearSpriteInfo();
	this->LoadSpriteFrames("Resources/Images/BeamShot_001.png", GL_CLAMP, GL_NEAREST);
...
void BeamShot::ReceiveMessage(Message *message)
{
