...
PierceShot::PierceShot(float X, float Y){
	//initialize PierceShot position and speed
	position.X = X;
	position.Y = Y;
	//PierceShotVel += shooterVel;
	//setup PierceShot
	this->SetPosition(position);
	this->SetSize(1.0f);
	this->SetColor(1, 1, 1, 1); //(white and opaque so the texture comes through fully)
	this->ClearSpriteInfo();
	this->LoadSpriteFrames("Resources/Images/PierceShot_001.png", GL_CLAMP, GL_NEAREST);
...
void PierceShot::Update(float dt){
	...
	//update PierceShot position according to its velocity
	position += velocity;
	//move PierceShot
...
void PierceShot::ReceiveMessage(Message *message)
{
